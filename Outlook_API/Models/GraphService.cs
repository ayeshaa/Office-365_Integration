...
{            
    ...
    public class GraphService
    {
        ...
        public async Task<string> DeleteEmail(string accessToken, string id)
        {

            // Get the current user. 
            // The app only needs the user's email address, so select the mail and userPrincipalName properties.
            // If the mail property isn't defined, userPrincipalName should map to the email for all account types. 
            string endpoint = "https://graph.microsoft.com/v1.0/me/messages/" + id;
            //string queryParameter = "?$select=mail,userPrincipalName";
            UserInfo me = new UserInfo();
        ...
        public async Task<string> ReplyEmail(string accessToken, string id, string coment)
        {

            // Get the current user. 
            // The app only needs the user's email address, so select the mail and userPrincipalName properties.
            // If the mail property isn't defined, userPrincipalName should map to the email for all account types. 
            string endpoint = "https://graph.microsoft.com/v1.0/me/messages/" + id + "/reply";
            //string queryParameter = "?$select=mail,userPrincipalName";
            UserInfo me = new UserInfo();
            ...
            using (var client = new HttpClient())
            {
                ...
                {
                    request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
                    request.Content = new StringContent(JsonConvert.SerializeObject(reply), Encoding.UTF8, "application/json");
        ...
        public async Task<string> ForwardEmail(string accessToken, string id, string ForEmail, string body)
        {

            // Get the current user. 
            // The app only needs the user's email address, so select the mail and userPrincipalName properties.
            // If the mail property isn't defined, userPrincipalName should map to the email for all account types. 
            string endpoint = "https://graph.microsoft.com/v1.0/me/messages/" + id + "/forward";
            //string queryParameter = "?$select=mail,userPrincipalName";
            UserInfo me = new UserInfo();
        ...
        public async Task<string> SendEmail(string accessToken, MessageRequest email)
        {
            ...
            //string endpoint = "https://graph.microsoft.com/v1.0/sendMail";
            ...
            using (var client = new HttpClient())
            {
                using (var request = new HttpRequestMessage(HttpMethod.Post, endpoint))
                {
                    ...
                    using (var response = await client.SendAsync(request))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            return "Success! Your mail was sent.";
                        }
        ...
        public async Task<MessageRequest> BuildEmailMessage(string accessToken, string recipients, string subject,string contentbody)
        {
            ...
            //{
               ...
               // }
                ...
                string bodyContent = string.Format(contentbody);
